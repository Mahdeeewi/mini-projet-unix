...
typedef struct {
    ...
    int        sockfd;        /* socket TCP vers le serveur mono-service */
    gboolean   authenticated; /* true après OK */
    time_t     conn_start;    /* début de la connexion */
} AppWidgets;
...
static void on_quit_session_clicked(GtkButton *button, gpointer user_data) {
    ...
    send_quit_and_close(app);
    gtk_label_set_text(GTK_LABEL(app->label_status),
                       "Session fermée. Vous pouvez vous reconnecter.");
    gtk_stack_set_visible_child_name(GTK_STACK(app->stack), "page_login");
}
...
int main(int argc, char *argv[]) {
    ...
    app = gtk_application_new("org.example.clientGUI.mono", G_APPLICATION_FLAGS_NONE);
